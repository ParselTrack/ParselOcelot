...
Schedule::Schedule(ConnectionMother *mother, Worker *worker, SiteComm *site_comm, Config *config) :
mother_(mother), worker_(worker), site_comm_(site_comm), config_(config) {
	counter_ = 0;
	last_opened_connections_ = 0;
	
	next_reap_peers_ = time(NULL) + config_->kReapPeersInterval + 40;
}
