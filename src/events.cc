...
ConnectionMother::ConnectionMother(Worker *worker, SiteComm *site_comm, Config *config) : worker_(worker), site_comm_(site_comm), config_(config) {
	open_connections_ = 0;
	...
	// Listen
	if(listen(listen_socket_, config_->kMaxConnections) == -1) {
	...
	// Create libev timer
	Schedule timer(this, worker_, site_comm_, config_);
	
	schedule_event_.set<Schedule, &Schedule::Handle>(&timer);
	schedule_event_.set(config_->kScheduleInterval, config_->kScheduleInterval); // After interval, every interval
...
ConnectionMiddleman::ConnectionMiddleman(int &listen_socket, sockaddr_in &address, socklen_t &addr_len, Worker *worker, ConnectionMother *mother, Config *config): 
	mother_(mother), worker_(worker), config_(config) {
	...
	timeout_event_.set(config_->kTimeoutInterval, 0);
